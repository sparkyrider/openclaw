...
- `store`：可选的 Gateway 网关 cron 存储文件路径。默认：`~/.openclaw/cron/jobs.json`。
- 实际的 cron 任务定义**不**应放在 `openclaw.json` 中。像 `cron.jobs` 这样的键属于无效配置；请通过 `openclaw cron ...` 或 cron 工具/API 创建和编辑任务。
